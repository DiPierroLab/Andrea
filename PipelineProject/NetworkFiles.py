...
for j in tsv:
    ...
    df = pd.DataFrame(data, columns=['ChromA', 'OG', 'GeneIndexA', 'GeneIndexAA', 'ChromB', 'OGB', 'GeneIndexB', 'GeneIndexBB', 'Score0', 'Score'])
    
    if(len(df) != 0):
    
        df = df.drop(['GeneIndexAA', 'GeneIndexBB', 'OGB' ,'Score0'], axis = 1)
        ...
        df_unsplit['GeneA'] = df_unsplit.apply(lambda row: str(j.split('-')[0] + '_G' + row['GeneIndexA']) + '_Ch' + row['ChromA'], axis=1)
        df_unsplit['GeneB'] = df_unsplit.apply(lambda row: str(j.split('-')[1] + '_G' + row['GeneIndexB']) + '_Ch' + row['ChromB'], axis=1)
        
        df_net = df_unsplit[['GeneA', 'GeneB', 'OG', 'Score']]
